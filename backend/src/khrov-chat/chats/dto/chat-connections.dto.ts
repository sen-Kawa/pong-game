...
export class ChatConnectionsDto {
  ...
  @IsPositive()
  @ApiProperty({
    description: 'userId of the user requesting to fetch a preview of all their existing chat conversations',
    example: 1,
  })
...
class Chat_historysType {
  ...
  @ApiProperty({
    description: 'The view is that of client1Id. Message will be outgoing if it was sent from client1Id',
    example: 'hello client2Id',
  })
  readonly outgoing: string;

  @ApiProperty({
    description: 'The view is that of client1Id. Message will be incoming if it was sent from client2Id',
...
export class ChatConnectionsResultDto {
  ...
  @ApiProperty({
    description: 'Each chat Conversation has two users invlved. This is the ID identifying the other user in same Chat conversation as unionId',
    example: 2,
  })
  ...
  @ApiProperty({
    description: 'blockStatus===true means this conversation between client1Id and client2Id is blocked',
    example: true,
  })
  readonly blockStatus: boolean;

  @ApiProperty({
    description: 'allowedToUnblock===true means it was client1Id that initiated the blocking and thus will be the user to also initiate the unblocking',
    example: true,
  })
