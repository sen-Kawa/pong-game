...
export class JoinOrExitChannelDto {
  ...
  @Matches(/^[a-zA-Z\d]*$/)
  @ApiProperty({
    description: 'Should be the right Password for Channel chId(if chId requires Password to access)',
    example: 'Abc123',
  })
  ...
  @IsBoolean()
  @ApiProperty({
    description: 'Should be either true or false. true means userId wants to join Channel chId. false means userId wants to exit Channel chId',
    example: true,
  })
