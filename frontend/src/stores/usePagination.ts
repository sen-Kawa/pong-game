...
export default function usePagination<Type>(items: Ref<Type[]>, pageSize: number) {
  if (pageSize <= 0) throw new Error('pageSize has to be positive')
  const currentPage = ref(1)

  const nextPage = () => {
    if (currentStartIndex.value + pageSize > items.value.length) return
    currentPage.value += 1
  }
  const prevPage = () => {
